...
const buildClient = () => {
  ...
  removeSync(join(buildDir, 'build'));
  console.log('Building client...');
  execSync('npm run build', { cwd: clientDir, stdio: 'inherit' });
  console.log('Copying client build to server...');
  copySync(join(clientDir, 'build'), join(buildDir, 'build'));
}

const buildServer = () => {
  console.log('Cleaning server build folder...');
  removeSync(join(buildDir, 'publish'));
  ...
  copySync(join(serverDir, 'bin', 'Release', 'net7.0', 'linux-x64', 'publish'), join(buildDir, 'publish'));
}
